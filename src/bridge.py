...
class StringField(Field):
    ...
    def validate(self, value):
        ...
        if value is not None:
            # Ideally we'd use six.ensure_text() here, but RHEL8 doesn't
            # have that
...
request_types = {
    ...
    'list-keys': RT(()),
    'new-key': RT((SF('key'),
                   SF('keytype', optional=True),
                   SF('initial-key-admin', optional=True),
    ...
    'sign-ostree': RT((SF('key'),
                      ...
                      max_payload=1024 * 1024 * 1024),
    'sign-rpm': RT((SF('key'),
                    SF('file-signing-key', optional=True),
                    SF('rpm-name', optional=True),
